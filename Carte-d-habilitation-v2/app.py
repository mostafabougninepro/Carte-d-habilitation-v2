...
# Recherche flexible de l'agent dans le Registre Excel
def get_agent_data(matricule):
    base_dir = os.path.dirname(os.path.abspath(__file__))

    # 1. Recherche du fichier Excel dans le dossier (gestion des minuscules/majuscules)
    excel_filename = None
    ...
    if not excel_filename:
        st.error(
            "⚠️ Fichier Excel du registre introuvable ! Vérifiez qu'il est bien présent sur GitHub."
        )
        return None
        ...
        for sheet_name in xl.sheet_names:
            ...
            if "Matricule" in df.columns:
                ...
                if not agent.empty:
                    ...
                    # 1. Vérification si Nom et Prénom sont séparés
                    ...
                    # 2. Recherche si Nom et Prénom sont regroupés
...
def generate_custom_excel():
    ...
    selected_name = template_map.get(selected_modele, "CL.xlsx")
    base_dir = os.path.dirname(os.path.abspath(__file__))

    # Recherche du template sans problème de casse (Case-Insensitive)
    tmpl_path = None
    for f in os.listdir(base_dir):
        if f.lower() == selected_name.lower():
            tmpl_path = os.path.join(base_dir, f)
            break

    if not tmpl_path:
        tmpl_path = os.path.join(base_dir, selected_name)
